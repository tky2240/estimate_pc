        ...
        for table_row in table_rows:
            try:
                ...
                motherboard_data_dictionary = {
                    ...
                    "pci_slot_count": 0 if table_data[19].text.strip() == "" else int(re.findall(r"(\d+)本", table_data[19].text.strip())[0]),
                    "pcie_x16_slot_count": 0 if table_data[15].text.strip() == "" else int(re.findall(r"(\d+)x PCI Express", table_data[15].text.strip())[0]),
                    "pcie_x8_slot_count": 0 if table_data[16].text.strip() == "" else int(re.findall(r"(\d+)x PCI Express", table_data[16].text.strip())[0]),
                    "pcie_x4_slot_count": 0 if table_data[17].text.strip() == "" else int(re.findall(r"(\d+)x PCI Express", table_data[17].text.strip())[0]),
                    "pcie_x1_slot_count": 0 if table_data[18].text.strip() == "" else int(re.findall(r"(\d+)x PCI Express", table_data[18].text.strip())[0]),
                    "sata_connector_count": 0 if table_data[20].text.strip() == "" else int(re.findall(r"(\d+)本", table_data[20].text.strip())[0]),
                    "release_date": None if table_data[42].text.strip() == "" else datetime.datetime.strptime(re.sub(r"\s", "", table_data[42].text), "%Y年%m月%d日").date()
                    if re.match(r"\d+年\d+月\d+日", re.sub(r"\s", "", table_data[42].text)) is not None else datetime.datetime.strptime(re.findall(r"(\d+年\d+月)", re.sub(r"\s", "", table_data[42].text))[0], "%Y年%m月").date(),
                    "is_exist": True
                }
            except Exception as exception:
                print(f"error with {exception}")
        ...
        for table_row in table_rows:
            try:
                ...
                gpu_data_dictionary = {
                    ...
                    "tdp": None if re.match(r"(\d+)W", table_data[19].text.strip()) is None else int(re.findall(r"(\d+)W", table_data[19].text.strip())[0]),
                    "hdmi_count": 0 if re.match(r"(\d+)ポート", table_data[26].text.strip()) is None else int(re.findall(r"(\d+)ポート", table_data[26].text.strip())[0]),
                    "displayport_count": 0 if re.match(r"(\d+)ポート", table_data[27].text.strip()) is None else int(re.findall(r"(\d+)ポート", table_data[27].text.strip())[0]),
                    "width": None if table_data[28].text.strip() == "" else float(re.findall(r"(\d+\.\d+|\d+)x(\d+\.\d+|\d+)x(\d+\.\d+|\d+)", table_data[28].text.strip())[0][0]),
                    "height": None if table_data[28].text.strip() == "" else float(re.findall(r"(\d+\.\d+|\d+)x(\d+\.\d+|\d+)x(\d+\.\d+|\d+)", table_data[28].text.strip())[0][1]),
                    "depth": None if table_data[28].text.strip() == "" else float(re.findall(r"(\d+\.\d+|\d+)x(\d+\.\d+|\d+)x(\d+\.\d+|\d+)", table_data[28].text.strip())[0][2]),
                    "radiator_width": None if table_data[29].text.strip() == "" else float(re.findall(r"(\d+\.\d+|\d+)x(\d+\.\d+|\d+)x(\d+\.\d+|\d+)", table_data[29].text.strip())[0][0]),
                    "radiator_height": None if table_data[29].text.strip() == "" else float(re.findall(r"(\d+\.\d+|\d+)x(\d+\.\d+|\d+)x(\d+\.\d+|\d+)", table_data[29].text.strip())[0][1]),
                    "radiator_depth": None if table_data[29].text.strip() == "" else float(re.findall(r"(\d+\.\d+|\d+)x(\d+\.\d+|\d+)x(\d+\.\d+|\d+)", table_data[29].text.strip())[0][2]),
                    "release_date": None if table_data[31].text.strip() == "" else datetime.datetime.strptime(re.sub(r"\s", "", table_data[31].text), "%Y年%m月%d日").date()
                    if re.match(r"\d+年\d+月\d+日", re.sub(r"\s", "", table_data[31].text)) is not None else datetime.datetime.strptime(re.findall(r"(\d+年\d+月)", re.sub(r"\s", "", table_data[31].text))[0], "%Y年%m月").date(),
                    "is_exist": True
                }
            except Exception as exception:
                print(f"error with {exception}")
        ...
        for table_row in table_rows:
            try:
                ...
                case_data_dictionary = {
                    ...
                    "weight": None if table_data[20].text.strip() == "" else float(re.findall(r"(\d+\.\d+|\d+)kg", table_data[20].text.strip())[0]),
                    "width": None if table_data[21].text.strip() == "" else float(re.findall(r"(\d+\.\d+|\d+)x(\d+\.\d+|\d+)x(\d+\.\d+|\d+)", table_data[21].text.strip())[0][0]),
                    "height": None if table_data[21].text.strip() == "" else float(re.findall(r"(\d+\.\d+|\d+)x(\d+\.\d+|\d+)x(\d+\.\d+|\d+)", table_data[21].text.strip())[0][1]),
                    "depth": None if table_data[21].text.strip() == "" else float(re.findall(r"(\d+\.\d+|\d+)x(\d+\.\d+|\d+)x(\d+\.\d+|\d+)", table_data[21].text.strip())[0][2]),
                    "release_date": None if table_data[25].text.strip() == "" else datetime.datetime.strptime(re.sub(r"\s", "", table_data[25].text), "%Y年%m月%d日").date()
                    if re.match(r"\d+年\d+月\d+日", re.sub(r"\s", "", table_data[25].text)) is not None else datetime.datetime.strptime(re.findall(r"(\d+年\d+月)", re.sub(r"\s", "", table_data[25].text))[0], "%Y年%m月").date(),
                    "is_exist": True
                }
        ...
        for table_row in table_rows:
            try:
                ...
                power_supply_data_dictionary = {
                    ...
                    "release_date": None if table_data[29].text.strip() == "" else datetime.datetime.strptime(re.sub(r"\s", "", table_data[29].text), "%Y年%m月%d日").date()
                    if re.match(r"\d+年\d+月\d+日", re.sub(r"\s", "", table_data[29].text)) is not None else datetime.datetime.strptime(re.findall(r"(\d+年\d+月)", re.sub(r"\s", "", table_data[29].text))[0], "%Y年%m月").date(),
                    "is_exist": True
                }
            except Exception as exception:
